...
async function a() {
    for (let filetype of ["TEMP", "TBLU", "ASET", "CPPT", "UICT", "UICB"]) {
        ...
        for (let rpkg of klaw(RUNTIME_PATH).map(a => a.path).filter(a => a.endsWith(".rpkg"))) {
        ...
        let allFiles = klaw("extraction").map(a => a.path).filter(a => a.endsWith(`.${filetype}`) || a.endsWith(`.${filetype}.meta`)).map(a => {
            return {
